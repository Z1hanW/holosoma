...
class ExportVideo(RL_EvalCallback):
    ...
    def write_recordings(self):
        ...
        if self.config.record_viewer:
            ...
            images = [img for img in os.listdir(self.config.viewer_record_dir) if img.endswith(".png")]
            images.sort()
            sample_frame = cv2.imread(os.path.join(self.config.viewer_record_dir, images[0]))
            height, width, layers = sample_frame.shape

            fourcc = SUFFIX_TO_FOURCC[self.config.suffix]
            fourcc = cv2.VideoWriter_fourcc(*fourcc)
            video = cv2.VideoWriter(str(self.record_dir / f"viewer_video.{self.config.suffix}"), fourcc, 30, (width, height))

            for image in images:
                video.write(cv2.imread(os.path.join(self.config.viewer_record_dir, image)))
