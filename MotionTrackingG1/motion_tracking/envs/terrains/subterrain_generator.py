...
from motion_tracking.envs.terrains.terrain_utils import perlin
from motion_tracking.envs.terrains.subterrain import SubTerrain
from scipy import interpolate
from motion_tracking.envs.utils.shape_utils import *


def random_uniform_subterrain(subterrain: SubTerrain, min_height, max_height, step=1, downsampled_scale=None, ):
    """
    ...
    height_field_downsampled = np.random.choice(heights_range,
                                                (int(subterrain.width * subterrain.horizontal_scale / downsampled_scale), int(
                                                    subterrain.length * subterrain.horizontal_scale / downsampled_scale)))

    x = np.linspace(0, subterrain.width * subterrain.horizontal_scale, height_field_downsampled.shape[0])
    y = np.linspace(0, subterrain.length * subterrain.horizontal_scale, height_field_downsampled.shape[1])

    f = interpolate.interp2d(y, x, height_field_downsampled, kind='linear')

    x_upsampled = np.linspace(0, subterrain.width * subterrain.horizontal_scale, subterrain.width)
    y_upsampled = np.linspace(0, subterrain.length * subterrain.horizontal_scale, subterrain.length)
...
def sloped_subterrain(subterrain: SubTerrain, slope=1):
    ...
    Parameters:
        subterrain (motion_tracking.envs.terrains.route_subterrain.RouteSubTerrain): the terrain
    ...
    max_height = int(slope * (subterrain.horizontal_scale / subterrain.vertical_scale) * subterrain.width)
    subterrain.height_field_raw[:, np.arange(subterrain.length)] += (max_height * xx / subterrain.width).astype(
...
def pyramid_sloped_subterrain(subterrain: SubTerrain, slope=1, platform_size=1.):
    """
    Generate a sloped terrain
    ...
    max_height = int(slope * (subterrain.horizontal_scale / subterrain.vertical_scale) * (subterrain.width / 2))
    subterrain.height_field_raw += (max_height * xx * yy).astype(subterrain.height_field_raw.dtype)
...
def discrete_obstacles_subterrain(subterrain: SubTerrain, max_height, min_size, max_size, num_rects, platform_size=1.):
    """
    ...
    for _ in range(num_rects):
        ...
        subterrain.height_field_raw[start_i:start_i + width, start_j:start_j + length] = max_height#np.random.choice(height_range)
...
def update_top_obstacles(map_description, subterrain):
    ...
    for obst in subterrain.top_obstacles:
        start_x = int((obst.get("cx") - (obst.get("length", 1.0) / 2)) / subterrain.horizontal_scale) - slack
        end_x = int((obst.get("cx") + (obst.get("length", 1.0) / 2)) / subterrain.horizontal_scale) + slack
        start_y = int((obst.get("cy") - (obst.get("width", 1.0) / 2)) / subterrain.horizontal_scale) - slack
        end_y = int((obst.get("cy") + (obst.get("width", 1.0) / 2)) / subterrain.horizontal_scale) + slack
        ...
        subterrain.ceiling_field_raw[start_x:end_x, start_y:end_y] = \
            np.minimum(subterrain.ceiling_field_raw[start_x:end_x, start_y:end_y], ceiling)
...
def update_static_obstacles(map_description, subterrain):
    ...
    for obst in subterrain.static_obstacles:
        if obst["type"] == "box":
            ...
            subterrain.height_field_raw[start_i:start_i + obs_size, start_j:start_j + obs_size] = obs_height
            subterrain.walkable_field_raw[start_i:start_i + obs_size, start_j:start_j + obs_size] = 1
...
def update_terrain(map_description, subterrain):
    if "terrain" in map_description:
        for terrain_desc in map_description["terrain"]:
            ...
            end_i = int(terrain_desc.get("end_x", subterrain.width) / subterrain.horizontal_scale)
            end_j = int(terrain_desc.get("end_y", subterrain.length) / subterrain.horizontal_scale)
            if terrain_desc["type"] == "gravel":
                amplitude = terrain_desc.get("amplitude", 0.05)
                width = end_i - start_i
                length = end_j - start_j
                gravel = np.int16(
                    (2 * amplitude * (np.random.random((width, length)) - 0.5)) / subterrain.vertical_scale)
            ...
            if terrain_desc["type"] == "sloped":
                ...
                tmp_hf[(tmp_hf.shape[0] // 2):, :] = tmp_hf[((tmp_hf.shape[0] // 2) - 1)::-1, :]
            ...
            if terrain_desc["type"] == "mixed":
                ...
                subterrain_stairs = stairs_subterrain(subterrain, step_width=0.5, step_height=0.075)
                ...
                subterrain.height_field_raw[:(slope_hf.shape[0] // 2), :] = slope_hf[:(slope_hf.shape[0] // 2), :]
                # add gravel
                amplitude = 0.04
                start_i = 0
...
def update_segmentation(map_description, subterrain):
    if "segmentation" in map_description:
        ...
        subterrain.seg_color = {seg["name"]: seg.get('color', 'none') for seg in segmentation}
                ...
                for seg in segmentation:
                    ...
                    if dist_to_seg < min_dist and dist_to_seg < radius:
                        ...
                        is_goal = seg.get("goal_radius") is None or \
                                  (dist_to_seg < (seg.get("goal_radius") / subterrain.horizontal_scale))
                if seg_name is None:
                    raise "No default terrain, fix segmentation!"
                subterrain.segmentation_field[(x, y)] = {"name": seg_name, "is_goal": is_goal}


def wave_subterrain(subterrain, num_waves=1, amplitude=1.):
    """
    ...
    if num_waves > 0:
        ...
        xx = xx.reshape(subterrain.width, 1)
        yy = yy.reshape(1, subterrain.length)
        subterrain.height_field_raw += (amplitude * np.cos(yy / div) + amplitude * np.sin(xx / div)).astype(
            subterrain.height_field_raw.dtype)
    return subterrain
...
def pyramid_stairs_subterrain(subterrain, step_width, step_height, platform_size=1.):
    """
    Generate stairs
...
def stepping_stones_subterrain(subterrain: SubTerrain, stone_size, stone_distance, max_height, platform_size=1., depth=-10):
    """
    ...
    if subterrain.length >= subterrain.width:
        while start_y < subterrain.length:
            ...
            subterrain.height_field_raw[0: stop_x, start_y: stop_y] = np.random.choice(height_range)
            # fill row
            while start_x < subterrain.width:
                stop_x = min(subterrain.width, start_x + stone_size)
                subterrain.height_field_raw[start_x: stop_x, start_y: stop_y] = np.random.choice(height_range)
        ...
        while start_x < subterrain.width:
            ...
            subterrain.height_field_raw[start_x: stop_x, 0: stop_y] = np.random.choice(height_range)
            # fill column
            while start_y < subterrain.length:
                stop_y = min(subterrain.length, start_y + stone_size)
                subterrain.height_field_raw[start_x: stop_x, start_y: stop_y] = np.random.choice(height_range)
...
def poles_subterrain(subterrain: SubTerrain, difficulty=1):
    ...
    probs = np.array([0.9, 0, 0.4, 0.5, 0.5]) * ((1 - base_prob) * difficulty + base_prob)
    low, high = 200, 500
        ...
        if i == 0:
            for _ in range(10 * num_mult):
                if np.random.binomial(1, p):
                    img += draw_disk(img_size=subterrain.width, max_r = 7) * int(np.random.uniform(low, high))
            ...
            for _ in range(3 * num_mult):
                if np.random.binomial(1, p):
                    img += draw_curve(img_size=subterrain.width) * int(np.random.uniform(low, high))
        elif i == 3 and np.random.binomial(1, p):
            for _ in range(1 * num_mult):
                if np.random.binomial(1, p):
                    img += draw_polygon(img_size=subterrain.width, max_sides=5) * int(np.random.uniform(low, high))
        elif i == 4 and np.random.binomial(1, p):
            for _ in range(5 * num_mult):
                if np.random.binomial(1, p):
                    img += draw_ellipse(img_size=subterrain.width,
                                        max_size=5) * int(
                                            np.random.uniform(low, high))
