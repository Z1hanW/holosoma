...
from smpllib.khrylib.mocap.skeleton_local import Skeleton
from smpllib.khrylib.mocap.skeleton_mesh_local import Skeleton as SkeletonMesh
...
from smpllib.utils.geom import quadric_mesh_decimation, center_scale_mesh
import uuid
import atexit
import shutil
import joblib
# from scipy.spatial.qhull import _Qhull
from smpllib.utils.flags import flags
import cv2
...
class Robot:
    ...
    def load_from_skeleton(
        ...
        size_dict = {}
        if self.mesh:
            # Store temporary generated meshes in a user-writable temp directory
...
if __name__ == "__main__":
    ...
    model = load_model_from_path(f"pacer/data/assets/mjcf/smpl_humanoid.xml")

    # smpl_robot.write_xml(f"/smpllib/smpl/smpl_humanoid_auto.xml")
    # model = load_model_from_path(f"/smpllib/smpl/smpl_humanoid_auto.xml")

    # smpl_robot.write_xml(f"test_good.xml")
    # model = load_model_from_path(f"test_good.xml")
    # model = load_model_from_path(f"pacer/data/assets/mjcf/amp_humanoid.xml")
