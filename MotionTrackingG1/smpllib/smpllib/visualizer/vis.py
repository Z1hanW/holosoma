...
def rescale_video(video_path, out_path, width=-1, height=-1, verbose=True):
    ...
    cmd = [FFMPEG_PATH, "-y", "-i", video_path, "-vf", f"scale={width}:{height}", out_path]
    if not verbose:
        cmd += ['-hide_banner', '-loglevel', 'error']
    subprocess.run(cmd)


def crop_video(video_path, out_path, width="iw", height="ih", x=0, y=0, crop_str=None, verbose=True):
    os.makedirs(osp.dirname(out_path), exist_ok=True)
    if crop_str is None:
...
def clip_video(video_path, out_path, start, end, verbose=True):
    os.makedirs(osp.dirname(out_path), exist_ok=True)
    cmd = [FFMPEG_PATH, "-y", "-ss", f"{start}", "-i", video_path, "-t", f"{end - start}", out_path]
    if not verbose:
        cmd += ['-hide_banner', '-loglevel', 'error']
    subprocess.run(cmd)


def text_video(video_path, out_path, text, x=10, y=30, verbose=True, text_color='white', text_size=60):
    os.makedirs(osp.dirname(out_path), exist_ok=True)
...
def make_gif(video_path, out_path, t_start='00:00:00', t_end=None, width=600, height=-1, verbose=True):
    os.makedirs(osp.dirname(out_path), exist_ok=True)
    cmd = [FFMPEG_PATH, "-y", "-i", video_path, "-ss", t_start] + (['-t', t_end] if t_end is not None else []) + \
          ["-vf", f"fps=30,scale={width}:{height}:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse",
           "-loop", "0", out_path]
    if not verbose:
        cmd += ['-hide_banner', '-loglevel', 'error']
    subprocess.run(cmd)
...
def images_to_video(img_dir, out_path, img_fmt="%06d.jpg", fps=30, crf=25, verbose=True):
    ...
    cmd = [FFMPEG_PATH, '-y', '-r', f'{fps}', '-f', 'image2', '-start_number', '0',
           '-i', f'{img_dir}/{img_fmt}', '-vcodec', 'libx264', '-vf', 'pad=ceil(iw/2)*2:ceil(ih/2)*2',
           '-crf', f'{crf}', '-pix_fmt', 'yuv420p', out_path]
    if not verbose:
        cmd += ['-hide_banner', '-loglevel', 'error']
...
def hstack_videos(video1_path, video2_path, out_path, crf=25, verbose=True, text1=None, text2=None, text_color='white',
                  text_size=60):
...
def vstack_videos(video1_path, video2_path, out_path, crf=25, verbose=True, text1=None, text2=None, text_color='white',
                  text_size=60):
    if not (text1 is None or text2 is None):
        write_text = True
        tmp_file = f'{osp.splitext(out_path)[0]}_tmp.mp4'
...
def vstack_video_arr(video_arr, out_path, crf=25, verbose=True, text_arr=None, text_color='white', text_size=60):
    ...
    if text_arr is not None:
        font_file = font_files[platform.system()]
        draw_str = ','.join(
            [f"drawtext=fontsize={text_size}:fontfile={font_file}:fontcolor={text_color}:text='{x}':x=10:y={h}+20" for
             h, x in zip(start_h, text_arr)])
...
def hstack_video_arr(video_arr, out_path, crf=25, verbose=True, text_arr=None, text_color='white', text_size=60):
    ...
    if text_arr is not None:
        font_file = font_files[platform.system()]
        draw_str = ','.join(
            [f"drawtext=fontsize={text_size}:fontfile={font_file}:fontcolor={text_color}:text='{x}':x={w}+10:y=20" for
             w, x in zip(start_w, text_arr)])
        cmd = [FFMPEG_PATH, '-i', tmp_file1, '-y', '-vf', draw_str, '-c:a', 'copy', out_path]
        if not verbose:
            cmd += ['-hide_banner', '-loglevel', 'error']
        subprocess.run(cmd)
...
def make_checker_board_texture(color1='black', color2='white', width=10, height=10, n_tile=15, to_bgr=False):
