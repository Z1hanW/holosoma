...
log_dir="${ROOT_DIR}/logs/real_ppo_first_h1_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$log_dir"
exec > >(tee -a "${log_dir}/run.log") 2>&1

echo "[real_ppo_first_h1] log_dir=${log_dir}"
...
python3 src/holosoma_inference/holosoma_inference/run_policy.py \
  inference:g1-root_pos-actions-no-linvel-h1 \
  --task.model-path _ckps/kxnhgj2v_model_07000.onnx \
  --task.use-joystick \
  --task.rl-rate 50 \
  --task.interface eth0
