...
checkpoint in:
  https://wandb.ai/zihanw22/boxer/runs/d9m3z369-recovered/files/model_22000.pt
...
if [[ "${AS_CONTACT_AWARE}" == "1" ]]; then
  ...
  if [[ "${AS_SUCCESS133_FINAL0P5}" == "1" && "${RESUME_FROM_BOX}" == "1" ]]; then
    ...
    export SCHEDULE_NOTES="${SCHEDULE_NOTES:-AS teacher-rollout filtered 133-clip real-mesh perception distill initialized from actor policy parameters in zihanw22/boxer/d9m3z369-recovered/model_22000.pt. Clips satisfy stable_contact_success=True and final_object_position_error_m<=0.5, use teacher-exported contact sidecars for offline contact guidance and adaptive contact-window sampling, and keep the PPO+DAgger hybrid active from iteration 0.}"
  elif [[ "${AS_SUCCESS133_FINAL0P5}" == "1" ]]; then
