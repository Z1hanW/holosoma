...
INFER_PYTHON_BIN="${INFER_PY:-/home/user/.holosoma_deps/miniconda3/envs/hsinference/bin/python}"
if [[ ! -x "$INFER_PYTHON_BIN" ]]; then
  INFER_PYTHON_BIN="/home/user/.holosoma_deps/miniconda3/envs/hssim/bin/python"
fi
if [[ ! -x "$INFER_PYTHON_BIN" ]]; then
...
def object_urdf_fallbacks(path: Path):
    ...
    if name:
        yield repo_root / "data_demo/objects" / f"{name}.urdf"
        if "__" in name:
...
export HOLOSOMA_DEFAULT_POSE_INIT="${HOLOSOMA_DEFAULT_POSE_INIT:-0}"
...
elif "perception_obs" in input_dims and obs_dim == 323 and actor_input_dim == [
    "actor_obs_root_contact_aware",
    "actor_obs_proprio",
]:
...
elif "perception_obs" in input_dims and obs_dim == 453 and actor_input_dim == [
    "actor_obs_root",
    "actor_obs_proprio_no_linvel",
    "actor_obs_actions",
]:
    print("g1-29dof-wbt-object-action-history-depth-distill")
elif "perception_obs" in input_dims and obs_dim == 468 and actor_input_dim == [
    "actor_obs_root",
    "actor_obs_proprio",
    "actor_obs_actions",
]:
