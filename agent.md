...
90. formal training launch 不再包含 motion-object replay 视频门，也不得在 worker/tmux 启动前录制或上传 `vis/replay`。source snapshot、effective motion bank、object map、rank-shard、motion-transition、checkpoint/teacher 和 ONNX 仍必须由各自 immutable manifest/digest 独立完成 fail-closed 校验；这些校验通过后直接创建/连接 W&B identity 并启动训练。视频或 reference replay 只在用户显式要求时作为启动路径之外的独立任务执行，其缺失或上传状态不得阻塞、延迟或改变 formal training。
...
96. “teacher from scratch”要求 `training_resume_enabled=false`、`policy_init_enabled=false`，actor、critic、两 optimizer、RNG 和 iteration 全部 fresh。未显式预分配 W&B identity 时由 logger 在训练启动时创建 fresh run；formal contract 预先固定但尚未创建的 run ID 必须用 `resume=never` 直接创建并在 ID 已存在时 fail closed。只有显式复用已有 logging identity 的独立合同才允许 `WANDB_RESUME=must`，且不得借此恢复模型状态。周期 checkpoint 固定每1000次 update，普通保存不得 reset live rollout。
