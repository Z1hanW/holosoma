...
default_model_path="${ROOT_DIR}/_ckps/reference/swl41n4x/model_20000.onnx"
if [[ ! -f "$default_model_path" ]]; then
  default_model_path="wandb://zihanw22/carry-any/swl41n4x/model_20000.onnx"
fi
model_path="${HOLOSOMA_REAL_CORL_MODEL_PATH:-${HOLOSOMA_REAL_MODEL_PATH:-$default_model_path}}"

log_dir="${ROOT_DIR}/logs/real_corl_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$log_dir"
exec > >(tee -a "${log_dir}/run.log") 2>&1
command_status_path="${log_dir}/latest_command.json"

echo "[real_corl] log_dir=${log_dir}"
