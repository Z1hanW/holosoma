...
class BasePolicy:
    ...
    def __init__(self, config: InferenceConfig):
        ...
        self._last_policy_cycle_has_state = False
        self._missing_low_state_warned = False
        self._viser_viewer = None
        self._viser_update_interval = 1
        self._viser_step_count = 0
        self._clip_joint_targets = str(os.getenv("HOLOSOMA_CLIP_JOINT_TARGETS", "0")).lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def policy_action(self):
        ...
        with self.latency_tracker.measure("read_state"):
            ...
            if robot_state_data is None:
                if not self._missing_low_state_warned:
                    self.logger.warning("Waiting for robot low-state stream before sending commands.")
                    self._missing_low_state_warned = True
                self.last_robot_state_data = None
                return
            if self._missing_low_state_warned:
                self.logger.info("Robot low-state stream received; enabling command loop.")
                self._missing_low_state_warned = False
            self.last_robot_state_data = robot_state_data
            self._last_policy_cycle_has_state = True
        ...
        with self.latency_tracker.measure("postprocessing"):
            ...
            # Optional safety clipping; disabled by default for parity with holosoma_inference_gt.
