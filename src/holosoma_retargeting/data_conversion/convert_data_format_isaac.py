...
parser = argparse.ArgumentParser(description="Replay motion from file (csv or npz), interpolate to correct frequency, then output to npz file.")
parser.add_argument("--input_file", type=str, required=True, help="The path to the input motion file.")
...
parser.add_argument("--once", action="store_true", default=False, help="Run the motion once and exit.")
...
# Pre-defined configs
##
from g1_config import G1_CYLINDER_CFG

def drake_convention_to_isaaclab_convention(
        pose: torch.Tensor
) -> torch.Tensor:
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        if self.motion_file.endswith(".csv"):
            ...
            if "q_robot" in data:
                motion = torch.from_numpy(data['q_robot']).to(torch.float32)
            else:
