...
__global__ void eig_val_kernel(const scalar_t *__restrict__ x, scalar_t *__restrict__ eig_val, size_t numel) {
  ...
  if (i < numel) {
    ...
    if (p1 == 0) {
      ...
      const scalar_t p2 = (x11 - q) * (x11 - q) + (x22 - q) * (x22 - q) + (x33 - q) * (x33 - q) + 2 * p1;
      ...
      scalar_t r =
          b11 * b22 * b33 + b12 * b23 * b31 + b13 * b21 * b32 - b13 * b22 * b31 - b12 * b21 * b33 - b11 * b23 * b32;
      r = r / 2.0;
...
template <typename scalar_t>
__global__ void eig_vec_kernel(const scalar_t *__restrict__ x, const scalar_t *__restrict__ eig_val,
...
std::tuple<at::Tensor, at::Tensor> sym3eig_cuda_forward(at::Tensor x) {
  ...
  AT_DISPATCH_FLOATING_TYPES(x.scalar_type(), "eig_kernel", [&] {
    eig_val_kernel<scalar_t><<<BLOCKS(eig_val.numel() / 3), THREADS>>>(
        x.data_ptr<scalar_t>(), eig_val.data_ptr<scalar_t>(), eig_val.numel() / 3);
    eig_vec_kernel<scalar_t><<<BLOCKS(eig_val.numel()), THREADS>>>(
        x.data_ptr<scalar_t>(), eig_val.data_ptr<scalar_t>(), eig_vec.data_ptr<scalar_t>(), eig_val.numel());
  });
...
template <typename scalar_t>
__global__ void eig_kernel_backward(const scalar_t *__restrict__ U_grad,  // Gradients of eig_vec
...
at::Tensor sym3eig_cuda_backward(
  ...
  AT_DISPATCH_FLOATING_TYPES(eig_vec.scalar_type(), "eig_kernel_backward", [&] {
    eig_kernel_backward<scalar_t><<<BLOCKS(eig_val.numel() / 3), THREADS>>>(eig_vec_grad.data_ptr<scalar_t>(),
        eig_vec.data_ptr<scalar_t>(), eig_val_grad.data_ptr<scalar_t>(), eig_val.data_ptr<scalar_t>(),
        x_grad.data_ptr<scalar_t>(), eig_val.numel() / 3);
  });
