...
#define SYM3EIG_FORWARD(MATRIX)                                                                                  \
  [&]() -> std::tuple<at::Tensor, at::Tensor> {                                                                  \
    ...
    auto eig_vec = at::empty_like(MATRIX);                                                                       \
                                                                                                                 \
    AT_DISPATCH_FLOATING_TYPES(MATRIX.scalar_type(), "sym3eig_fw", [&] {                                         \
      auto matrix_data = MATRIX.data_ptr<scalar_t>();                                                            \
      auto eig_vec_data = eig_vec.data_ptr<scalar_t>();                                                          \
      auto eig_val_data = eig_val.data_ptr<scalar_t>();                                                          \
      for (ptrdiff_t i = 0; i < num_matrices; i++) {                                                             \
        ...
        if (p1 == 0) {                                                                                           \
          ...
          const scalar_t b33 = (1.0 / p) * (x33 - q);                                                            \
                                                                                                                 \
          scalar_t r = b11 * b22 * b33 + b12 * b23 * b31 + b13 * b21 * b32 - b13 * b22 * b31 - b12 * b21 * b33 - \
                       b11 * b23 * b32;                                                                          \
          r = r / 2.0;                                                                                           \
                                                                                                                 \
          scalar_t phi;                                                                                          \
...
std::tuple<at::Tensor, at::Tensor> sym3eig_cpu_forward(at::Tensor matrix) {
...
#define SYM3EIG_BACKWARD(EIG_VEC_GRAD, EIG_VEC, EIG_VAL_GRAD, EIG_VAL)              \
  [&]() -> at::Tensor {                                                             \
    ...
    auto X_grad = at::empty_like(EIG_VEC_GRAD);                                     \
                                                                                    \
    AT_DISPATCH_FLOATING_TYPES(EIG_VEC.scalar_type(), "sym3eig_cpu_backward", [&] { \
      auto U = EIG_VEC.data_ptr<scalar_t>();                                        \
      auto U_grad = EIG_VEC_GRAD.data_ptr<scalar_t>();                              \
      auto S = EIG_VAL.data_ptr<scalar_t>();                                        \
      auto S_grad = EIG_VAL_GRAD.data_ptr<scalar_t>();                              \
      auto X_grad_data = X_grad.data_ptr<scalar_t>();                               \
                                                                                    \
      for (ptrdiff_t i = 0; i < num_matrices; i++) {                                \
        const int m_idx = 9 * i;                                                    \
        const int v_idx = 3 * i;                                                    \
