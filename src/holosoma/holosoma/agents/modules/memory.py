...
from holosoma.utils.torch_utils import unpad_trajectories
...
class Memory(nn.Module):
    ...
    def forward(self, memory_input, masks=None, hidden_states=None):
        batch_mode = masks is not None
        if batch_mode:
            # batch mode: needs saved hidden states
            if hidden_states is None:
                raise ValueError("Hidden states not passed to memory module during policy update")
            out, _ = self.rnn(memory_input, hidden_states)
            out = unpad_trajectories(out, masks)
        else:
            # inference/distillation mode: uses hidden states of last step
            out, self.hidden_states = self.rnn(memory_input.unsqueeze(0), self.hidden_states)
        return out
    ...
    def detach_hidden_states(self, dones=None):
        if self.hidden_states is not None:
            ...
            elif isinstance(self.hidden_states, tuple):  # tuple in case of LSTM
                for hidden_state in self.hidden_states:
