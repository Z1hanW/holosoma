...
def teacher_rollout_object_global_ref_position_error_exp(
    ...
    active_mask = _reward_episode_mask(
        ...
        only_external=only_external,
        only_clip_goal=only_clip_goal,
    ) & sampled_reference["object_valid_mask"]
    return reward * active_mask.to(dtype=torch.float32)
...
class OfflineContactPointGuidance(RewardTermBase):
    ...
    def __init__(self, cfg: RewardTermCfg, env: WholeBodyTrackingManager):
        ...
        self.only_command_env = bool(cfg.params.get("only_command_env", False))
    ...
    def __call__(self, env: WholeBodyTrackingManager, **kwargs) -> torch.Tensor:
        ...
        if self.only_command_env:
            reward = reward * self.motion_command.get_command_only_env_mask().to(dtype=torch.float32)
