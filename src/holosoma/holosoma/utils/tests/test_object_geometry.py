...
@pytest.mark.parametrize(
    ...
    [
        Path(
            str(Path(__file__).resolve().parents[5] / "data/ds_box_data/scale_mix_all/train_g1_w_obj_prepared/_clip_object_urdf_map.json")
        ),
        Path(
            str(Path(__file__).resolve().parents[5] / "data/ds_box_data/scale_mix_all/train_g1_w_obj_prepared_plus_omomo_orig/_clip_object_urdf_map.json")
        ),
    ],
)
...
def test_omomo_largebox_mesh_urdf_uses_primitive_override():
    urdf_path = Path(
        str(Path(__file__).resolve().parents[5] / "src/holosoma/holosoma/data/motions/g1_29dof/whole_body_tracking/objects_largebox.urdf")
    )
