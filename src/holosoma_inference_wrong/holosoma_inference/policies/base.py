...
class BasePolicy:
    ...
    def __init__(self, config: InferenceConfig):
        ...
        self.last_viser_state_data = None
    ...
    def _reset_observation_history(self) -> None:
        """Clear observation history so the next rollout starts from a clean state."""
        for group_buffers in self.obs_history_buffers.values():
            for history in group_buffers.values():
                history.clear()
        for obs_buf in self.obs_buf_dict.values():
            obs_buf.fill(0.0)

    def _reset_rollout_buffers(self) -> None:
        """Reset rollout-side buffers that should not leak across simulator resets."""
    ...
    def _maybe_update_viser(self) -> None:
        if self._viser_viewer is None or self.last_viser_state_data is None:
            return
        if self._viser_step_count % self._viser_update_interval == 0:
            self._viser_viewer.update(self.last_viser_state_data)
    ...
    def _on_run_exit(self) -> None:
        ...
        if self._viser_viewer is not None:
            close_viewer = getattr(self._viser_viewer, "close", None)
    ...
    def policy_action(self):
        ...
        with self.latency_tracker.measure("read_state"):
            ...
            self.last_viser_state_data = self._get_viser_state_data(robot_state_data)
