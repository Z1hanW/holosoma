...
class ViserInferenceViewer:
    def __init__(self, robot_config: RobotConfig, cfg: ViserConfig, model_path: str) -> None:
        ...
        self.object = None
        if cfg.object_urdf_path:
            object_urdf_path = Path(cfg.object_urdf_path)
            if not object_urdf_path.exists():
                raise FileNotFoundError(f"Viser object URDF path does not exist: {object_urdf_path}")
            self.object_root = self.server.scene.add_frame("/object", show_axes=False)
            self.object = ViserUrdf(self.server, urdf_or_path=object_urdf_path, root_node_name="/object")
            self.object.show_visual = cfg.show_meshes
            self._object_enabled = True
        ...
        def _(_evt) -> None:
            ...
            if self.object is not None:
                self.object.show_visual = bool(show_meshes_cb.value)
    ...
    def update(self, payload) -> None:
        ...
        if isinstance(payload, dict):
            state_payload = payload.get("robot_state_data")
            object_pos_w = payload.get("object_pos_w")
            object_quat_wxyz = payload.get("object_quat_wxyz")
        if state_payload is None:
            return

        state = np.asarray(state_payload)
        if state.ndim == 1:
        ...
        if self._object_enabled and object_pos_w is not None and object_quat_wxyz is not None:
            object_pos = np.asarray(object_pos_w, dtype=np.float32).reshape(-1, 3)[0]
            object_quat = np.asarray(object_quat_wxyz, dtype=np.float32).reshape(-1, 4)[0]
            assert self.object_root is not None
            self.object_root.position = object_pos
            self.object_root.wxyz = object_quat
