...
> **UNSUPPORTED / NON-SCIENTIFIC ARCHIVE:** This directory is a historical snapshot with known
> deployment-contract bugs. It must not be installed or used for experiments, simulation, or robot
> deployment. Use `src/holosoma_inference`. Import and installation are blocked by default; the exact
> escape hatch `HOLOSOMA_ALLOW_UNSAFE_ARCHIVAL_INFERENCE=1` exists only for explicit historical forensics.
