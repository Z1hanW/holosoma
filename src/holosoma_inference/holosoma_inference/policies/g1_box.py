...
class G1BoxPolicy(BasePolicy):
    ...
    def __init__(self, config: InferenceConfig):
        ...
        self._sparse_root_command = self._parse_sparse_root_command(
            os.environ.get("HOLOSOMA_BOX_SPARSE_ROOT_COMMAND", "")
        )
        ...
        self._keyboard_command_yaw = abs(
            float(
                os.environ.get(
                    "HOLOSOMA_BOX_COMMAND_YAW",
                    np.deg2rad(float(os.environ.get("HOLOSOMA_KEYBOARD_ROOT_COMMAND_YAW_DEGREES", "17.0"))),
                )
            )
        )
        super().__init__(config)
        self._log_sparse_root_command("Initial")
        stiff_q = config.robot.stiff_startup_pos or config.robot.default_dof_angles
        stiff_kp = config.robot.stiff_startup_kp or config.robot.motor_kp
        stiff_kd = config.robot.stiff_startup_kd or config.robot.motor_kd
        self._stiff_hold_q = np.asarray(stiff_q, dtype=np.float32).reshape(1, -1)
        self._stiff_hold_kp = np.asarray(stiff_kp, dtype=np.float32)
        self._stiff_hold_kd = np.asarray(stiff_kd, dtype=np.float32)
        if self._stiff_hold_q.shape[1] != self.num_dofs:
            raise ValueError("G1 box stiff startup pose dimension mismatch with robot DOFs")
    ...
    def _parse_sparse_root_command(raw: str) -> np.ndarray:
        ...
        if raw:
            try:
                ...
                logger.warning("Ignoring invalid HOLOSOMA_BOX_SPARSE_ROOT_COMMAND='{}'", raw)
        return np.zeros((1, 3), dtype=np.float32)

    def _log_sparse_root_command(self, prefix: str = "Current") -> None:
        command = tuple(float(v) for v in self._sparse_root_command.reshape(-1)[:3])
        if command == self._last_logged_sparse_root_command and prefix == "Current":
            return
        self._last_logged_sparse_root_command = command
        logger.info(
            "{} sparse root command: x={:.3f} y={:.3f} yaw={:.3f} rad ({:.1f} deg)",
            prefix,
            command[0],
            command[1],
            command[2],
            float(np.rad2deg(command[2])),
        )
    ...
    def handle_keyboard_button(self, keycode):
        ...
        if self._set_sparse_root_command_from_key(key):
            return
    ...
    def _get_sparse_root_command(self) -> np.ndarray:
        return self._sparse_root_command.copy()
