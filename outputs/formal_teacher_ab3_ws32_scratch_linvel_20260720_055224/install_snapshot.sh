...
readonly SNAPSHOT_ID=src-a8879814e247e78b0e686fb8bdf94b985d008b2960aa187b482ef8de7b644399
readonly MANIFEST_SHA=a8879814e247e78b0e686fb8bdf94b985d008b2960aa187b482ef8de7b644399
readonly ARCHIVE_SHA=dcbf88cdc3c274799240b078a52fb7019a3bd0637e2339422fae6287f3357de2
readonly ARCHIVE=/home/ubuntu/FAR/holosoma_runs/.snapshots/src-a8879814e247e78b0e686fb8bdf94b985d008b2960aa187b482ef8de7b644399.tar.gz
...
for node in "$@"; do
  ...
  ssh -o BatchMode=yes -o ConnectTimeout=10 "${node}" "mkdir -p '${RUNS_ROOT}/.incoming'"
  scp -q -o BatchMode=yes -o ConnectTimeout=10 "${ARCHIVE}" "${node}:${remote_archive}"
...
verify_snapshot() {
  local root=$1 link_path link_target asset_path source_dir
  [[ -d "${root}" && ! -L "${root}" ]]
...
chmod u+w "${temporary}"
mkdir -p "${temporary}/.checkpoint_cache" "${temporary}/.teacher_checkpoints" \
  "${temporary}/.run_control" "${temporary}/logs/batch_ne"
chmod 700 "${temporary}/.checkpoint_cache" "${temporary}/.teacher_checkpoints" \
  "${temporary}/.run_control" "${temporary}/logs" "${temporary}/logs/batch_ne"
