...
Optional env vars:
  INFER_DATASET           (default: omomo; options: rollout-ref|teacher-rollout|omomo|behave|mixed)
...
DEFAULT_OMOMO_MOTION_DIR="${SCRIPT_DIR}/data/ds_box_data/train_g1_w_obj_prepared"
...
DEFAULT_OMOMO_URDF="${DEFAULT_OMOMO_MOTION_DIR}/_clip_object_urdf_map.json"
...
if [[ "${MOTION_DIR_EXPLICIT}" -eq 0 && -n "${CHECKPOINT_SAVED_MOTION_PATH}" && "${INFER_DATASET_EXPLICIT}" -eq 0 && "${INFER_DATASET}" != "rollout-ref" && "${INFER_DATASET}" != "omomo" ]]; then
  MOTION_DIR="${CHECKPOINT_SAVED_MOTION_PATH}"
fi
if [[ "${OBJECT_URDF_EXPLICIT}" -eq 0 && -n "${CHECKPOINT_SAVED_OBJECT_URDF}" && "${INFER_DATASET_EXPLICIT}" -eq 0 && "${INFER_DATASET}" != "rollout-ref" && "${INFER_DATASET}" != "omomo" ]]; then
