...
from motion_tracking.agents.models.discriminator import (
    DISC_LOGIT_INIT_SCALE
)
from motion_tracking.agents.models.mlp import MLP_WithNorm
...
class NormalizedLinearEncoder(nn.Module):
    def __init__(self, input_dim, output_dim, normalization):
        ...
        torch.nn.init.uniform_(self.enc.weight, -ENC_LOGIT_INIT_SCALE, ENC_LOGIT_INIT_SCALE)
...
class JointDiscWithMutualInformationEncMLP(MLP_WithNorm):
    def __init__(self, config, num_in: int, num_out: int = 1):
        ...
        for idx, latent_dim in enumerate(config.mi_enc.latent_dim):
            if config.mi_enc.latent_types[idx] == "gaussian":
                ...
                enc = NormalizedLinearEncoder(config.shared.out_dim, latent_dim, normalization="von Mises-Fisher")
            elif config.mi_enc.latent_types[idx] == "uniform":
                enc = GaussianEncoder(config.shared.out_dim, latent_dim)
            elif config.mi_enc.latent_types[idx] == "categorical":
                enc = NormalizedLinearEncoder(config.shared.out_dim, latent_dim, normalization="softmax")
            else:
                raise NotImplementedError
            encoders.append(enc)
    ...
    def forward(self, input_dict, already_normalized=False, return_norm_obs=False, return_enc=False):
        obs = (
