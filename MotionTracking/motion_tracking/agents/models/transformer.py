...
from motion_tracking.agents.models.mlp import build_mlp, MLP_WithNorm
from motion_tracking.utils.model_utils import get_activation_func
...
class TransformerWithNorm(nn.Module):
    def __init__(self, config, num_in: int, num_out: int):
        ...
        self.obs_encoder = MLP_WithNorm(config.obs_mlp, num_in, config.latent_dim - config.type_embedding_dim)
        num_entries = 1
        ...
        for key in self.extra_input_keys:
            ...
            model = instantiate(
                config.extra_inputs[key], num_in=num_in
            )
            ...
            for operation in self.config.extra_inputs[key].config.get("operations", []):
                if operation.type == "embedding_per_entry":
                    embedding_per_entry = Embedding(
                        num_embeddings=operation.num_embeddings, embedding_dim=operation.embedding_dim
                    )
        ...
        # Transformer layers
        self.sequence_pos_encoder = PositionalEncoding(config.latent_dim - config.type_embedding_dim)
        ...
        self.seqTransEncoder = nn.TransformerEncoder(seqTransEncoderLayer, num_layers=config.num_layers)
        # Map transformer layer to actions
        self.output_mapping = build_mlp(self.config.output_mlp, config.latent_dim, num_out)

    def get_extracted_features(self, input_dict, already_normalized=False, return_norm_obs=False):
        type_array = torch.arange(len(self.extra_input_keys) + 1, dtype=torch.long, device=input_dict["obs"].device)

        batch_size = input_dict["obs"].shape[0]

        cur_encoded_state = self.obs_encoder({"obs": input_dict["obs"]}, already_normalized, return_norm_obs)
        if return_norm_obs:
            cur_encoded_state, obs = cur_encoded_state["outs"], cur_encoded_state["norm_obs"]

        cur_state_embedding = self.type_embedding(type_array[0].unsqueeze(0)).expand(cur_encoded_state.shape[0], -1)
        cat_obs = torch.cat([cur_encoded_state, cur_state_embedding], dim=1).unsqueeze(1)

        # cur_mask = torch.zeros(batch_size, 1, dtype=torch.bool, device=input_dict["obs"].device)
        ...
        for key_idx, key in enumerate(self.extra_input_keys):
            ...
            for operation in self.config.extra_inputs[key].config.get("operations", []):
                if operation.type == "permute":
                    ...
                    embedding_per_entry_module = self.extra_input_models[key + "_" + operation.name]
                    key_type_array = torch.arange(embedding_per_entry_module.num_embeddings, dtype=torch.long,
                                                  device=input_dict["obs"].device)
                    key_obs = torch.cat([key_obs, embedding_per_entry_module(key_type_array.unsqueeze(0)).expand(
                        key_obs.shape[0], -1, -1)], dim=-1)
                elif operation.type == "type_embedding":
                    key_type_embedding = self.type_embedding(type_array[key_idx + 1].unsqueeze(0).unsqueeze(0)).expand(
                        key_obs.shape[0], key_obs.shape[1], -1)
                    ...
                    key_obs = key_obs * input_dict[self.mask_keys[key]].view(*input_dict[self.mask_keys[key]].shape, *((1,) * extra_needed_dims))
                    ...
                    extra_needed_dims = num_obs_dims - num_mask_dims
                    key_obs = key_obs * input_dict[self.mask_keys[key]].view(*input_dict[self.mask_keys[key]].shape, *((1,) * extra_needed_dims))
                    key_obs = torch.cat([key_obs, input_dict[self.mask_keys[key]].view(*input_dict[self.mask_keys[key]].shape, *((1,) * extra_needed_dims))], dim=-1)
                    used_mask_multiply = True
                elif operation.type == "maxpooling":
                    key_obs = torch.max(key_obs, dim=1)[0].unsqueeze(1)
                elif operation.type == "maxpooling_add":
                    key_obs += torch.max(key_obs, dim=1)[0].unsqueeze(1)
                else:
            ...
            #     if not self.config.extra_inputs[key].config.get("mask_valid_as_zeros", True):
            #         key_mask = key_mask.logical_not()
            # else:
            #     key_mask = torch.zeros(batch_size, key_obs.shape[1], dtype=torch.bool, device=input_dict["obs"].device)
        ...
        output = self.seqTransEncoder(cat_obs)[0]  # [bs, d]
    ...
    def forward(self, input_dict, already_normalized=False, return_norm_obs=False):
        ...
        output = self.get_extracted_features(input_dict, already_normalized, return_norm_obs)
        if return_norm_obs:
...
class Embedding(nn.Module):
    # TODO: replace this with models.common.Embedding
