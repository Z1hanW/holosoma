...
def get_object_heightmap(mesh, dim_x: int = 10, dim_y: int = 10, dim_multiplier: int = 10):
...
if __name__ == "__main__":
    ...
    bad_objects = []
    for obj_path in list(Path("../../data/assets/urdf/objects/train/LargeSofas").rglob("*.obj")) + list(Path("../../data/assets/urdf/objects/test/LargeSofas").rglob("*.obj")):
        print(obj_path)
        ...
        max_w = max(w_x, w_y)
        heightmap = get_object_heightmap(mesh, dim_x=int(np.round(w_x / 0.1)), dim_y=int(np.round(w_y / 0.1)),
                                         dim_multiplier=10)
    ...
    max_w = max(w_x, w_y)
    heightmap = get_object_heightmap(mesh, dim_x=int(np.round(w_x / 0.1)), dim_y=int(np.round(w_y / 0.1)), dim_multiplier=10)
