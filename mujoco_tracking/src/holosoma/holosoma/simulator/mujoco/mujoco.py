...
class MuJoCo(BaseSimulator):
    ...
    def setup_viewer(self) -> None:
        ...
        self._update_cotracking_viewer_camera()
        self._update_text_overlay()
        logger.info("=== Viewer setup completed with keyboard callback ===")

    def _update_cotracking_viewer_camera(self) -> None:
        """Keep the MuJoCo viewer framed on both the robot and carried object."""
        if self.viewer is None or self.root_model is None or self.root_data is None:
            return
        if os.environ.get("HOLOSOMA_COTRACKING_CAMERA", "1").strip().lower() in {"0", "false", "no", "off"}:
            return

        robot_pos = np.asarray(self.root_data.xpos[1], dtype=np.float64)
        object_positions = []
        for body_id in getattr(self, "_object_mujoco_body_ids", set()):
            if 0 <= int(body_id) < self.root_model.nbody:
                object_positions.append(np.asarray(self.root_data.xpos[int(body_id)], dtype=np.float64))

        if object_positions:
            object_pos = np.mean(np.stack(object_positions, axis=0), axis=0)
            target = 0.5 * (robot_pos + object_pos)
            separation = float(np.linalg.norm(robot_pos[:2] - object_pos[:2]))
        else:
            target = robot_pos
            separation = 0.0
        ...
        self.viewer.cam.distance = max(3.0, 2.4 + 0.8 * separation)
