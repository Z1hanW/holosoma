...
class WholeBodyTrackingPolicy(BasePolicy):
    def __init__(self, config: InferenceConfig):
        ...
        self._refresh_observation_feature_flags(obs_terms)
        ...
        self._uses_onnx_motion_outputs = False
    ...
    def _refresh_observation_feature_flags(self, obs_terms: set[str] | None = None) -> None:
        if obs_terms is None:
            obs_terms = {term for terms in self.config.observation.obs_dict.values() for term in terms}
        ...
        self._uses_legacy_object_obs = (
            ...
            and "obj_target_pos_b" not in obs_terms
        )
    ...
    def _maybe_enable_actor_obs_terms_from_metadata(self, metadata: dict) -> None:
        ...
        object_terms = [term for term in actor_terms if term in OBJECT_OBS_DIMS]
        if not object_terms:
            return

        obs_dict = {group: list(terms) for group, terms in self.config.observation.obs_dict.items()}
        ...
        unknown_terms = [term for term in actor_terms if term not in obs_dims and term not in OBJECT_OBS_DIMS]
        if unknown_terms:
            logger.warning("Dropping unsupported actor obs terms from ONNX metadata: {}", unknown_terms)
        actor_terms_filtered = [term for term in actor_terms if term not in unknown_terms]
        for term in actor_terms_filtered:
            if term in OBJECT_OBS_DIMS:
                obs_dims[term] = int(OBJECT_OBS_DIMS[term])
                obs_scales.setdefault(term, 1.0)

        obs_dict["actor_obs"] = actor_terms_filtered
        history_lengths["actor_obs"] = 1
        object.__setattr__(
            self.config,
            "observation",
            ObservationConfig(
        ...
        self._refresh_observation_feature_flags()
        logger.info("Enabled actor_obs terms from ONNX metadata: {}", actor_terms_filtered)
    ...
    def setup_policy(self, model_path):
        ...
        self._maybe_enable_actor_obs_terms_from_metadata(metadata)
        ...
        self._uses_onnx_motion_outputs = required_motion_outputs.issubset(self._motion_output_names)
        if self._uses_motion_command and not self._uses_onnx_motion_outputs:
            logger.warning(
                "ONNX has no motion outputs; using motion clip data for motion_command/motion_ref_ori_b. "
                "Available outputs: {}",
                self.onnx_output_names,
            )

        self._onnx_output_fetch = [self._action_output_name]
        if self._uses_motion_command and self._uses_onnx_motion_outputs:
        ...
        if self._uses_motion_command and self._uses_onnx_motion_outputs:
            ...
            or self._uses_videomimic
            or self._uses_object_generalist
            or self._uses_legacy_object_obs
            or self._uses_sparse_root_command
        ) and self._motion_data is not None:
            self._sync_motion_outputs_from_motion_data(0)
            self.motion_command_0 = self.motion_command_t.copy()
            self.ref_quat_xyzw_0 = self.ref_quat_xyzw_t.copy()
        elif (
            self._uses_videomimic
    ...
    def _sync_motion_outputs_from_motion_data(self, motion_index: int) -> None:
        if self._motion_data is None:
            return
        idx = max(0, min(int(motion_index), int(self._motion_data.frame_count) - 1))
    ...
    def _sync_motion_outputs_from_onnx(self, motion_index: int) -> None:
        ...
        if not self._uses_motion_command:
            return
        if not self._uses_onnx_motion_outputs:
            self._sync_motion_outputs_from_motion_data(motion_index)
            return
        if self._time_step_input_name is None:
            return
    ...
    def _get_object_generalist_obs_buffer_dict(self, robot_state_data: np.ndarray) -> dict[str, np.ndarray]:
        ...
        return {
            ...
            "obj_size": object_size.astype(np.float32, copy=False),
