...
if [[ -z ${EXPECTED_COMMIT} || ! ${EXPECTED_COMMIT} =~ ^[0-9a-f]{40}$ ]]; then
  echo "usage: $0 <full-commit-sha> [run-label]" >&2
  exit 2
fi
...
echo "[INFO] nonformal_hmi_canary commit=${EXPECTED_COMMIT} world_size=8 total_envs=512 iterations=2 export_onnx=true"
