...
def _canonicalize_one_link_urdf(src_urdf: Path, dst_urdf: Path) -> None:
    ...
    if len(links) != 1:
        raise ValueError(
            f"single-slot canonicalization requires exactly one URDF link; {src_urdf} has {len(links)}"
        )
...
def prepare_single_slot_map(*, motion_dir: Path, object_map: Path, output_map: Path) -> tuple[int, int]:
    ...
    output_payload["single_slot_canonicalization"] = {
        ...
        "notes": "Generated for public-release single-slot metadata; mesh paths remain file-backed.",
    }
