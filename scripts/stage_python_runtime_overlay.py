...
roots cover numerical arrays, configuration semantics, and the complete ONNX
export/checker/CPU-runtime stack. ``attrs`` is explicit because OmegaConf
probes it at import time even though OmegaConf does not declare it in
Requires-Dist.
...
def _import_root_for_file(relative: PurePosixPath) -> str | None:
    ...
    if len(parts) > 1:
        ...
        if (
            parts[-1] == "__init__.py"
            and all(_IMPORT_NAME_RE.fullmatch(part) for part in package_parts)
        ):
            # A distribution such as protobuf owns google/protobuf/__init__.py
            # while intentionally omitting google/__init__.py.  The first
            # package component is still its auditable import root.
            return top
        return None
...
def stage(site_packages: Path) -> dict[str, Any]:
    ...
    namespace_import_roots: set[str] = set()
    ...
    for canonical_name in sorted(records):
        ...
        for declared_path in sorted(declared_files, key=lambda value: str(value)):
            ...
            if import_root is not None:
                ...
                if len(pure_relative.parts) > 2 and pure_relative.name == "__init__.py":
                    namespace_import_roots.add(import_root)
    ...
    # Seal namespace-package roots inside the overlay.  Otherwise Python would
    # merge (for example) overlay/google with a heterogeneous node-local
    # site-packages/google directory, defeating the exact import closure even
    # though protobuf's own files were authenticated.
    for import_root in sorted(namespace_import_roots):
        initializer = site_packages / import_root / "__init__.py"
        if initializer.exists():
            if not initializer.is_file() or initializer.is_symlink():
                raise StagingError(
                    f"namespace import root is not a regular package: {initializer}"
                )
            continue
        initializer.parent.mkdir(parents=True, exist_ok=True)
        initializer.write_text(
            "# Generated by stage_python_runtime_overlay.py to seal this namespace.\n",
            encoding="ascii",
        )
