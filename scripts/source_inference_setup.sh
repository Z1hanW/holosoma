#!/usr/bin/env bash
CLIP="${1:-${HOLOSOMA_MJ_CLIP:-box_75}}"
CLIP="${CLIP%.npz}"
export HOLOSOMA_MJ_CLIP="$CLIP"
export HOLOSOMA_MJ_MOTION="${HOLOSOMA_MJ_MOTION:-data_demo/${CLIP}.npz}"
export HOLOSOMA_MJ_MODEL="${HOLOSOMA_MJ_MODEL:-https://wandb.ai/zihanw22/boxer/runs/w5qostjn}"
if [ -z "${HOLOSOMA_POLICY_MOTION_INDEX_OFFSET:-}" ]; then
    [ "$CLIP" = "box_75" ] && export HOLOSOMA_POLICY_MOTION_INDEX_OFFSET=1 || export HOLOSOMA_POLICY_MOTION_INDEX_OFFSET=0
fi

export PYTHONSAFEPATH=1
export PYTHONPATH="src/holosoma_inference:src/holosoma${PYTHONPATH:+:$PYTHONPATH}"
export SIM_CLOCK_PORT="${SIM_CLOCK_PORT:-5655}"
export SIM_STATE_PORT="${SIM_STATE_PORT:-5657}"
export PERCEPTION_OBS_PORT="${PERCEPTION_OBS_PORT:-5658}"
export SIM_CONTROL_PORT="${SIM_CONTROL_PORT:-5659}"
export SPARSE_ROOT_COMMAND_PORT="${SPARSE_ROOT_COMMAND_PORT:-5661}"
export PERCEPTION_OBS_SHM_NAME="${PERCEPTION_OBS_SHM_NAME:-depth_img_shm_${SIM_STATE_PORT}}"
export AUTO_START_STIFF_HOLD_SEC="${AUTO_START_STIFF_HOLD_SEC:-0.0}"
export AUTO_START_STIFF_MAX_WAIT_SEC="${AUTO_START_STIFF_MAX_WAIT_SEC:-0.0}"
export AUTO_START_STIFF_POSE_TOL="${AUTO_START_STIFF_POSE_TOL:-0.12}"
export ENABLE_EXTERNAL_SPARSE_ROOT_COMMAND="${ENABLE_EXTERNAL_SPARSE_ROOT_COMMAND:-1}"
export USE_ROOT_REFERENCE_AT_CLIP_START="${USE_ROOT_REFERENCE_AT_CLIP_START:-1}"
export POLICY_ACTION_SCALE="${POLICY_ACTION_SCALE:-0.25}"
export POLICY_RL_RATE="${POLICY_RL_RATE:-50}"
export HOLOSOMA_FORCE_MOTION_ALIGNMENT="${HOLOSOMA_FORCE_MOTION_ALIGNMENT:-1}"
export HOLOSOMA_CLIP_JOINT_TARGETS="${HOLOSOMA_CLIP_JOINT_TARGETS:-0}"
export HOLOSOMA_PREFILL_OBS_HISTORY_ON_MOTION_START="${HOLOSOMA_PREFILL_OBS_HISTORY_ON_MOTION_START:-0}"
export HOLOSOMA_ZMQ_LOWCMD_LATCH_CONTROL_BOUNDARY="${HOLOSOMA_ZMQ_LOWCMD_LATCH_CONTROL_BOUNDARY:-0}"
export HOLOSOMA_ZMQ_LOWCMD_LOCKSTEP_CONTROL_BOUNDARY="${HOLOSOMA_ZMQ_LOWCMD_LOCKSTEP_CONTROL_BOUNDARY:-1}"
export HOLOSOMA_ZMQ_LOWCMD_MATCH_TOLERANCE_MS="${HOLOSOMA_ZMQ_LOWCMD_MATCH_TOLERANCE_MS:-2}"
export HOLOSOMA_POLICY_TARGET_ROBOT_ROOT_STATE_ASSIST="${HOLOSOMA_POLICY_TARGET_ROBOT_ROOT_STATE_ASSIST:-0}"
export HOLOSOMA_POLICY_TARGET_ROBOT_DOF_STATE_ASSIST="${HOLOSOMA_POLICY_TARGET_ROBOT_DOF_STATE_ASSIST:-0}"
export HOLOSOMA_POLICY_TARGET_OBJECT_STATE_ASSIST="${HOLOSOMA_POLICY_TARGET_OBJECT_STATE_ASSIST:-0}"
export HOLOSOMA_POLICY_TTY_INPUT="${HOLOSOMA_POLICY_TTY_INPUT:-1}"
export HOLOSOMA_KEYBOARD_ROOT_COMMAND="${HOLOSOMA_KEYBOARD_ROOT_COMMAND:-1}"
export HOLOSOMA_SKIP_STIFF_PROMPT="${HOLOSOMA_SKIP_STIFF_PROMPT:-1}"
