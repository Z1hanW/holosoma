#!/usr/bin/env python3
"""Compare MuJoCo reset modes for the tracking web demo and report ground penetration."""
...
def _set_state(
    *,
    ...
    object_body_name = config.get("object_body_name", "largebox_link")
...
def _analyze_mode(model: mj.MjModel, data: mj.MjData) -> dict:
    geoms = []
...
def main() -> None:
    ...
    for mode in ("demo_raw", "isaac_training_default_pose"):
        state_summary = _set_state(model=model, data=data, config=config, mode=mode)
        analysis = _analyze_mode(model, data)
        print(mode)
        print(f"  root_pos: {state_summary['root_pos'].tolist()}")
        print(f"  root_quat_wxyz: {state_summary['root_quat_wxyz'].tolist()}")
        print(f"  joint_l2_vs_default: {state_summary['joint_l2_vs_default']:.6f}")
        print("  lowest geoms:")
        for row in analysis["geoms"][: args.top_k]:
            print(
                "    "
                f"{row['body_name']:<28} "
                f"{row['geom_name']:<32} "
                f"lowest_z={row['lowest_z']:.6f}"
            )
        print("  nearest contacts:")
        if analysis["contacts"]:
            for row in analysis["contacts"][: args.top_k]:
                print(
                    "    "
                    f"{row['geom1']} <-> {row['geom2']} dist={row['dist']:.6f}"
                )
        else:
            print("    none")
        print()
