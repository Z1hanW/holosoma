...
def main() -> None:
    ...
    if config["motion"].get("initial_object_pos_w") is not None:
        object_body_id = mj.mj_name2id(model, mj.mjtObj.mjOBJ_BODY, config.get("object_body_name", "largebox_link"))
        if object_body_id >= 0 and model.body_jntnum[object_body_id] > 0:
            ...
            with np.load(motion_path, allow_pickle=True) as motion_data:
                object_pos_w = np.asarray(motion_data["object_pos_w"], dtype=np.float64)
                object_quat_w = np.asarray(motion_data["object_quat_w"], dtype=np.float64)
