...
if is_truthy "${WEB_DEMO_MATCH}"; then
  ...
  export PERCEPTION_UPDATE_HZ="${PERCEPTION_UPDATE_HZ:-30.0}"
  export PERCEPTION_CAMERA_FPS="${PERCEPTION_CAMERA_FPS:-30.0}"
  export PERCEPTION_CAMERA_NEAR="${PERCEPTION_CAMERA_NEAR:-0.3}"
  export PERCEPTION_CAMERA_FAR="${PERCEPTION_CAMERA_FAR:-3.0}"
  export PERCEPTION_MAX_DISTANCE="${PERCEPTION_MAX_DISTANCE:-3.0}"
  export PERCEPTION_CAMERA_WARP_MIN_VALID_DEPTH="${PERCEPTION_CAMERA_WARP_MIN_VALID_DEPTH:-0.3}"
...
if is_truthy "${WEB_DEMO_MATCH}"; then
  export MUJOCO_OBJECT_CONTACT_BODY_MARKERS="${MUJOCO_OBJECT_CONTACT_BODY_MARKERS:-[\"rubber_hand\"]}"
else
...
if [[ "${PERCEPTION_CAMERA_SOURCE}" == "rendered" ]]; then
  ...
  export HOLOSOMA_MUJOCO_LOAD_OBJECT_VISUAL_MESHES="${HOLOSOMA_MUJOCO_LOAD_OBJECT_VISUAL_MESHES:-${WEB_DEMO_MATCH}}"
  export HOLOSOMA_MUJOCO_DEPTH_PREFER_OBJECT_VISUAL_MESHES="${HOLOSOMA_MUJOCO_DEPTH_PREFER_OBJECT_VISUAL_MESHES:-${WEB_DEMO_MATCH}}"
  export HOLOSOMA_MUJOCO_WEB_DEMO_OBJECT_CONTACTS="${HOLOSOMA_MUJOCO_WEB_DEMO_OBJECT_CONTACTS:-${WEB_DEMO_MATCH}}"
...
echo "[INFO] sim_fps=${SIM_FPS:-mj_track_default} control_decimation=${SIM_CONTROL_DECIMATION:-mj_track_default} action_hz=${POLICY_RL_RATE:-50} perception_update_hz=${PERCEPTION_UPDATE_HZ:-preset} camera_fps=${PERCEPTION_CAMERA_FPS:-preset}"
