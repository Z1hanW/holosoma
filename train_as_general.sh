...
case "$(echo "${AS_OBJECT_GEOMETRY_MODE}" | tr '[:upper:]' '[:lower:]')" in
  mesh|urdf)
    ...
    ;;
  *)
    echo "[ERROR] train_as_general.sh only supports mesh object geometry." >&2
    echo "[ERROR] Primitive/box/disabled geometry is not allowed for AS real-mesh training." >&2
    echo "[ERROR] Got OBJECT_GEOMETRY_MODE=${AS_OBJECT_GEOMETRY_MODE}" >&2
    exit 2
    ;;
esac
case "$(echo "${OBJECT_SPAWN_MODE}" | tr '[:upper:]' '[:lower:]')" in
  single_slot_multi_urdf|single-slot-multi-urdf|single_slot|single-slot|heterogeneous_single_slot|heterogeneous-single-slot)
    ...
    ;;
  *)
    echo "[ERROR] train_as_general.sh only supports OBJECT_SPAWN_MODE=single_slot_multi_urdf." >&2
